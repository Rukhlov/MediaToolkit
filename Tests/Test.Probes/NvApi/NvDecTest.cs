...
namespace Test.Probe.NvApi
{
    class NvDecTest
    {
        ...
        public void Start(string fileName, MfVideoArgs inputArgs)
        {
            ...
            deviceD3D11 = new SharpDX.Direct3D11.Device(adapter, deviceCreationFlags, featureLevel);
            factory.Dispose();
        ...
        private unsafe CuCallbackResult VideoDisplayCallback(IntPtr data, IntPtr infoPtr)
        {
            ...
            {
                //if (!_framesChannel.Writer.TryWrite(FrameInformation.FinalFrame))
                //{
            ...
            var frame = _decoder.MapVideoFrame(info.PictureIndex, ref processingParam, out var pitch);
            ...
            fixed (CuGraphicsResource* resPtr = resources)
            {
                ...
                LibCuda.GraphicsMapResources(2, resPtr, stream);
                ...
                LibCuda.GraphicsUnmapResources(2, resPtr, stream);
            }
