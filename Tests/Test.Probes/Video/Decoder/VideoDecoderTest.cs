	...
	class VideoDecoderTest
	{
		...
		private void ProcessCPUSample(Sample s, long sampleTime)
		{
			...
			using (var b = s.ConvertToContiguousBuffer())
			{
				...
				MediaFactory.GetStrideForBitmapInfoHeader((int)format, size.Width, out var stride);
				...
				var rowPitch =  stride;//4 * size.Width;
				//var slicePitch = rowPitch * size.Height;

				SharpDX.DataBox[] initData =
				{
					new SharpDX.DataBox((IntPtr)_ptr,  rowPitch, 0),
				};

				var tex = new Texture2D(device3D11, texDescr, initData);
