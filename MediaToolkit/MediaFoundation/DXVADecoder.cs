...
using Device3D9 = SharpDX.Direct3D9.Device;
using SharpDX.MediaFoundation.DirectX;

namespace MediaToolkit.MediaFoundation
{

    public class DXVADecoder
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();

        public Device3D9 device = null;
        ...
        public DXVADecoder(IntPtr hwnd)
        {
        ...
        public void Setup(MfVideoArgs inputArgs)
        {
            ...
            try
            {
                ...
                device = new DeviceEx(direct3D, adapterIndex, SharpDX.Direct3D9.DeviceType.Hardware, hWnd, createFlags, presentParams);
                ...
                using (var attr = decoder.Attributes)
                {
                    ...
                    if (d3dAware)
                    {
                        using (Direct3DDeviceManager devMan = new Direct3DDeviceManager())
                        {
                            
                            devMan.ResetDevice(device, devMan.CreationToken);
                            attr.Set(SinkWriterAttributeKeys.D3DManager, devMan);
                            attr.Set(SourceReaderAttributeKeys.DisableDxva, 0);
